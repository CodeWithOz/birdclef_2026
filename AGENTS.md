...
| 2026-05 | fastai's `Learner.to_parallel()` is not available on Kaggle. Use `torch.nn.DataParallel` directly: `learn.model = torch.nn.DataParallel(learn.model)`. Unwrap before export: `if isinstance(learn.model, torch.nn.DataParallel): learn.model = learn.model.module`. P100 is Pascal (sm_60) and is broken on Kaggle since PyTorch 2.8 dropped Pascal support in CUDA 12.8 builds. |
